...
import { ProductionItem, StoreItemDetails } from '../branchAdmin.model';
...
export class ProductionItemsComponent
  ...
  cloneFormGroup(dt: ProductionItem): UntypedFormGroup {
    return this.fb.group({
      id: [dt.id],
      title: [dt.title],
  ...
  LoadItemQuantity(item: ProductionItem) {
    this.http
      .list('store', 'productItem/ballnce/0/' + item.id + '/' + this.cmYear.id)
      .subscribe((e: any) => {
  ...
  onSubmit() {
    ...
    if (!dt.salePrice) dt.salePrice = 0;
  ...
  deleteItem(i: number, row: ProductionItem) {
  ...
  curQuantity = 0;
  detailsProject(item: ProductionItem) {
    this.caption = item.title;

    this.http
      .list('store', 'productItem/ballnce/1/' + item.id + '/' + this.cmYear.id)
      .subscribe((e: any) => {
        item.quantity = e.quantity;
        ...
        console.log(item);
        let cabbalance = item.initCabsa;

        const t = {} as StoreItemDetails;
        t.balance = item.initQuantity;
        t.blnsCabsa = item.initCabsa;
        ...
        item.details.forEach((k: any) => {
          ...
          if (k.type.trim() == 'FatoraItems') {
            w.type = 'بيع';
            balance -= k.data.quantity;
            cabbalance -= k.data.cabsa;
            w.qtCabsa = k.data.cabsa;
            w.comments =
              'المشتري: ' +
          ...
          if (k.type.trim() == 'ProductionOperation') {
            w.type = 'انتاج';
            balance += k.data.quantity;
            cabbalance += k.data.cabsa;
            w.qtCabsa = k.data.cabsa;
            w.comments =
              'اسم المنتج: ' +
              k.data.productionItemTitle +
              'اسم العامل: ' +
              k.data.workerName;
          }
          w.balance = balance;
          w.blnsCabsa = cabbalance;
